...
That setup is the harness core — isolated sub-agents, a structured ask gate, skill visibility, $ skill mention, a long-objective gate, side chat, context budgeting, code simplification review, web research, FFF search, a workflow engine for sub-agent fan-out, and ponytail discipline review. A three-package optional tier (targeted LSP diagnostics, interactive shell overlays, autonomous research loops) installs on request with `--only optional`. Other extras (skill arguments, memory, MCP, themes) install on demand with `pi install`.
